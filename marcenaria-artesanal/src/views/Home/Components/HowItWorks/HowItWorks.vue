        ...
        >
          ...
          >
            <div class="hiw-cardInner">
              ...
              <h3 class="hiw-cardTitle" v-html="step.title" />
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      // Garantir que começa no primeiro card no mobile
      if (this.$refs.row) this.$refs.row.scrollLeft = 0;
    });
  },
  beforeUnmount() {
  ...
  methods: {
    cardStride() {
      ...
      if (!row) return 0;
      const first = row.querySelector(".hiw-card");
      if (!first) return 0;
    ...
    scroll(dir = 1) {
      ...
      const target = Math.max(
        0,
        Math.min(row.scrollLeft + dir * stride, row.scrollWidth - row.clientWidth)
      );
      row.scrollTo({ left: target, behavior: "smooth" });
      // Atualiza setas após a animação
...
.hiw-left {
  left: 0.5rem; /* fora da área dos cards */
}
.hiw-right {
  right: 0.5rem;
}
