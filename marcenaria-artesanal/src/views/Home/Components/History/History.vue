<template>
  ...
  <section class="history-bg">
    <div class="hist" data-hist>
      ...
      >
        <div class="ov-dialog">
          <button class="ov-close" @click="closeVideo" aria-label="Fechar">✕</button>
          <div class="ov-player">
            <iframe
              class="player"
              :src="embedPlaySrc"
      ...
      <!-- ===== PARTE 2 — TEXTO (alinhado à esquerda) + AUTOR + MÉTRICAS ===== -->
        ...
        <div class="quote-wrap">
          ...
          <div class="quote-author vertical">
            ...
            <div class="author-meta">
              <div class="author-name">Elias Boy</div>
...
export default {
  ...
  methods: {
    ...
    handleKey(e) {
      if (e.key === 'Escape' && this.videoOpen) this.closeVideo()
    }
  },
  mounted() {
...
.history-bg{
  --alt-bg:#f3f4f7; /* ajuste aqui se quiser mais claro/escuro */
...
[data-hist]{
  ...
  max-width: 1400px;
  margin: 0 auto;
  padding: 84px 5% 76px; /* leve aumento de respiro */
...
.hero-video .frame{
  ...
  max-width: 1160px;              /* ↑ maior no desktop */
  aspect-ratio:16/9;
  border-radius:18px;
  background:
...
@media (min-width: 1440px){
  .hero-video .frame{ max-width: 1320px; }  /* ainda mais amplo em telas grandes */
}
.hero-video .ring{
...
.hero-video .thumb:hover img{ transform:scale(1.015); } /* hover mais sutil */
.hero-video .play{
...
@media (max-width: 640px){
  .hero-video .play{ width:60px; height:60px; line-height:60px; font-size:24px; }
}

/* Modal */
.ov{ position:fixed; inset:0; background:rgba(0,0,0,.65); backdrop-filter: blur(1px);
  display:grid; place-items:center; z-index:9999; }
.ov-dialog{ position:relative; width:min(1280px, calc(100% - 32px)); aspect-ratio:16/9;
  border-radius:14px; overflow:hidden; background:#000; box-shadow:0 18px 48px rgba(0,0,0,.35); }
.ov-player,.player{ width:100%; height:100%; border:0; display:block; }
.ov-close{ position:absolute; top:-44px; right:0; width:38px; height:38px; border-radius:999px; border:0;
  background:#fff; color:#111827; cursor:pointer; }

/* Texto alinhado à esquerda (premium) */
.quote-wrap{ max-width:1080px; margin:0 auto; text-align:center; }
.quote-text.single{
  text-align:left; margin:0 auto; max-width:74ch;
  font-size:clamp(16px,1.5vw,18px); line-height:1.9; color:var(--ink-800);
  letter-spacing:.0045em; text-wrap:pretty; hyphens:auto;
}
.quote-text.single .lead{ display:inline; font-size:clamp(18px,1.8vw,20px); font-weight:600; color:var(--ink-900); }
...
.more-btn{
  margin:14px auto 0; border:0; background:transparent; font-size:13px;
  letter-spacing:.12em; text-transform:uppercase; color:#111; opacity:.7; cursor:pointer;
}
.more-btn:hover{ opacity:1; }

/* Respiros */
.soft-sep{ height:46px; }   /* texto → logo */
.soft-sep.lg{ height:42px; }/* logo → card */
...
.author-role{ font-size:12px; letter-spacing:.04em; color:rgba(0,0,0,.6); }

/* Card de métricas (divisores apenas no desktop) */
.stats-card.dark{
  margin:0 auto; width:100%; max-width:1080px;
  background: radial-gradient(80% 140% at 50% -30%, rgba(255,255,255,.10), transparent 60%), var(--panel-dark);
  color: var(--panel-dark-ink); border:1px solid rgba(255,255,255,.08);
  padding:28px 18px; display:grid; grid-template-columns:repeat(3,1fr);
...
.stat{ display:grid; gap:10px; place-items:center; padding: 0 18px; }
.value{
  font-family:'Arboria-Thin', Arial, sans-serif; font-variant-numeric: tabular-nums;
  font-size:clamp(52px,6.8vw,82px); line-height:1; letter-spacing:-0.02em; color:#fff;
  text-shadow:0 2px 10px rgba(0,0,0,.35);
}
.label{ font-size:12px; letter-spacing:.14em; text-transform:uppercase; color:rgba(255,255,255,.72); }
.divider{ display:none; }
@media (min-width: 960px){
  .divider{ display:block; width:1px; height:56px; background:linear-gradient(180deg, rgba(255,255,255,.22), rgba(255,255,255,.08)); }
}

/* Acessibilidade: reduz movimento */
@media (prefers-reduced-motion: reduce){
  .hero-video .thumb img{ transition:none; }
}
...
@media (max-width:640px){
  ...
  .ov-dialog{ width:calc(100% - 20px); border-radius:12px; }
  .ov-close{ top:-40px; }
  .author-logo{ width:48px; height:48px; }
}
</style>
